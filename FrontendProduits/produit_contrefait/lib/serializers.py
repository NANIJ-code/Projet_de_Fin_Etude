...
class AlerteInconnueSerializer(serializers.ModelSerializer):
    message = serializers.CharField()

    class Meta:
        model = Alerte
        fields = ['message', 'date_alerte']

    def create(self, validated_data):
        emetteur = self.context.get('emetteur')
        destinataire = self.context.get('destinataire')
        alerte = Alerte.objects.create(
            message=validated_data['message'],
            emetteur=emetteur,
            destinataire=destinataire,
        ...
        if destinataire and destinataire.email:
            context = {
                'alerte_message': validated_data['message'],
                'emetteur_nom': emetteur.username,
                'emetteur_email': emetteur.email,
            }
            html_content = render_to_string('emails/alerte_inconnue_email.html', context)
            subject = "[MediScan] Alerte sur un produit inconnu"
            email = EmailMultiAlternatives(
                subject=subject,
